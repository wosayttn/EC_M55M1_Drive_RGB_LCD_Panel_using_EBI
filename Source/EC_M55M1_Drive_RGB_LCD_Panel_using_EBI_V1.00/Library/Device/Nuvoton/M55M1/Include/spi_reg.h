...
typedef struct
{
     ...
     * |[4]     |SLV3WIRE  |Slave 3-wire Mode Enable Bit (SPI Slave mode Only)
     ...
     * |        |          |0 = Uncompleted RX data will be dropped from RX FIFO when bit count error event happened in SPI sSlave mode.
     * |        |          |1 = Uncompleted RX data will be written into RX FIFO when bit count error event happened in SPI sSlave mode
     ...
     * |        |          |For SPI1~SPI4, the MSB of this bit field is only meaningful while SPI mode 4~16 bits of data length.
     ...
     * |        |          |This status register indicates that effective bit number of uncompleted RX data when SLVBERX (SPIx_FIFOCTL[10]) is enabled and RX bit count error event happen in SPI sSlave mode.
     ...
     * |        |          |10 = PCM mode A.
     * |        |          |11 = PCM mode B.
     * |[31]    |SLVERRIEN |Bit Clock LossNumber Error Interrupt Enable Bit for Slave Mode
     * |        |          |Interrupt occurs if this bit is set to 1 and bit clock lossnumber error event occurs.
     * |        |          |0 = Interrupt Disabled.
     * |        |          |1 = Interrupt Enabled.
     * @var SPI_T::I2SCLK
     ...
     * |        |          |User sets I2SSLAVE to set frequency of peripheral clock of I2S mMaster mode and I2S sSlave mode when BCLKDIV (SPIx_I2SCLK[17:8]) is set.
     * |        |          |I2SSLAVE needs to set before I2SEN (SPIx_I2SCTL[0]) is enabled.
     * |        |          |0 = The frequency of peripheral clock is set to I2S mMaster mode.
     * |        |          |1 = The frequency of peripheral clock is set to I2S sSlave mode.
     * @var SPI_T::I2SSTS
     ...
     * |[22]    |SLVERRIF  |Bit ClockNumber Loss Error Interrupt Flag for Slave Mode
     * |        |          |0 = No bit clock lossnumber error event occurred.
     * |        |          |1 = Bit clock lossnumber error event occurred.
     * |        |          |Note: This bit will be cleared by writing 1 to it.
     * |[23]    |TXRXRST   |TX or RX Reset Status (Read Only)
     * |        |          |0 = The reset function of TXRST or RXRST is done.
     * |        |          |1 = Doing the reset function of TXRST or RXRST.
