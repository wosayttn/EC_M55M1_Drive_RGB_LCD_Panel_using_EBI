...
typedef struct
{
     ...
     * |        |          |0 = set Set the QSPIx_SS line to inactive state.
     ...
     * |        |          |0 = The QSPI data out is keept 0 if there is TX underflow event in Slave mode.
     * |        |          |1 = The QSPI data out is keept 1 if there is TX underflow event in Slave mode.
     ...
     * |[10]    |SLVBERX   |RX FIFO Write Data Enable Bit When Slave Mode Bit Count Error (QSPI Slave mode Only)
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[29:24] |SLVBENUM  |Effective Bit Number of Uncompleted RX data (QSPI Slave mode Only)
     ...
     * |        |          |Note: Slave mode Only and Tthis register will be cleared to 0x0 when user write 0x1 to SLVBEIF (QSPIx_STATUS[6]).
     * @var QSPI_T::TX
...
#define QSPI_CTL_QSPIEN_Pos              (0)                                               /*!< QSPI_T::CTL: SPIEN Position            */
#define QSPI_CTL_QSPIEN_Msk              (0x1ul << QSPI_CTL_QSPIEN_Pos)                    /*!< QSPI_T::CTL: SPIEN Mask                */
...
#define QSPI_STATUS_QSPIENSTS_Pos        (15)                                              /*!< QSPI_T::STATUS: SPIENSTS Position      */
#define QSPI_STATUS_QSPIENSTS_Msk        (0x1ul << QSPI_STATUS_QSPIENSTS_Pos)              /*!< QSPI_T::STATUS: SPIENSTS Mask          */
